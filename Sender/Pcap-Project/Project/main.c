...
// Radili: Branislav Novak RA221/2015
//		   Stevan Popov RA207/2015
// ================================================================
...
unsigned char keyString[] = "ORM2Proj";
...
void *ethernetThreadFunction(void *params, int size_of_last);
/***********/
int countEthernet = 0;
int countWifi = 0;
...
int main()
{
	...
	//unsigned char* data_ext = "song_test.mp3";
	unsigned char* data_ext = "picture_test.jpg";
	//unsigned char* data_ext = "tekst_test.txt";
	...
	/************ SENDING HALF OF THE FILE OVER WIFI AND HALF OVER ETHERNET ************/

	pthread_create(&wifiThread, NULL, wifiThreadFunction, NULL);
	pthread_create(&ethernetThread, NULL, ethernetThreadFunction, NULL, size_of_last);
...
void *wifiThreadFunction(void *params)
{
	...
	//for (i = 0; i < number_of_packets; i++)								// wifi onlyl
	for (i = number_of_packets / 2; i < number_of_packets; i++)				// half eth half wifi
	{
		...
		//if (i == number_of_packets)					// wifi only
		if (i == number_of_packets - 1)					// half eth half wifi
		{
...
void *ethernetThreadFunction(void *params, int size_of_last) {
	//for (int i = 0; i < number_of_packets; i++)				// ethenet only
	for (int i = 0; i < number_of_packets / 2; i++)		// half eth half wifi
	{
		flagEthernetWorks = 1;
		countEthernet = 0;
		...
		//if (i != number_of_packets)						// ethernet only
		if (i != number_of_packets - 1)						// half eth half wifi
		{
			pthread_mutex_lock(&myMutex);
